...
module.exports = function(express, config){
	...
	.delete(function(req, res, next){
		// delete a result
		suExec('rm', ['rm', reqDir + '/' + req.reqId], function(err){
	...
	.put(function(req, res, next){
		...
		fs.writeFileSync(tmpFile, req.body, function(err){
			if(err) {
				res.sendStatus(403);
				return;
			}
			var destFile = mntDir + path.normalize('/' + req.path);
			suExec('mkdir', ['mkdir', '-p', fs.dirname(destFile)], function(){
				suExec('mv', ['mv', '-T', tmpFile, destFile], function(err, status, signal){
					...
					suExec('chown', ['chown', 'root:root', destFile], function(){
						res.sendStatus(200);
					});
				});
			});
	...
	.delete(function(req, res, next){
		suExec('rm', ['rm', mntDir + path.normalize('/' + req.path)], function(err){
			if(err) res.sendStatus(500);
			else res.sendStatus(200);
		});
	});
