...
exports.createEnvironment = function(root, options, cb){
	...
	// the exports
	var env = {
		exec: function(file, cb){
			// mount tmpfs
			var c = (options.tmp || defaultOptions.tmp).length + 1;
			var lastErr = null;
			var end = function(){
				if(lastErr) return cb( new Error('Failed to execute in "' + root + '".') );
				// call exec
				// TODO
			};
	...
	suExec.execPath('mkdir', ['mkdir', root], function(){
		var c = (options.bind || defaultOptions.bind).length + (options.robind || defaultOptions.robind).length + 1;
		var lastErr = null;
		var end = function(){
			cb(null, env);
		};
