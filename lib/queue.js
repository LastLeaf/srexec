...
var processPipeline = function(config, reqId, pipeline, env, envId, cb){
	...
	var nextStep = function(){
		...
		// mount
		var mntSrc = path.normalize('mnt' + path.resolve('/', item.mnt));
		env.mnt(mntSrc, function(err){
			// run in env
			env.exec({
				chroot: root,
				workingDir: item.workingDir || '/mnt',
	...
	// prepare env
	env.reset(function(){
		suExec.execPath('mkdir', ['mkdir', tmpFilesRoot], function(){
			suExec.execPath('rm', ['rm', 'req/' + reqId], function(){
				nextStep();
			});
		});
	});
...
var filterPipeline = function(pipeline, config){
	...
	for(var i=0; i<pipeline.length; i++) {
		...
		var r = {
			...
			mnt: String(item.mnt || ''),
			workingDir: String(item.workingDir || ''),
