...
var processPipeline = function(config, pipeline, env, envId, cb){
	...
	var nextStep = function(){
		...
		env.exec({
			...
			// handling stdout
			if(curStdout === execRoot + '/stdout') {
				suExec.execPath('mv', ['mv', curStdout, execRoot + '/stdin'], function(){
					curStdio = execRoot + '/stdin';
					nextStep();
				});
			} else {
				nextStep();
	...
	// prepare env
	env.reset(function(){
		fs.emptyDir(execRoot, function(err){
			nextStep();
		});
	});
};
